...
/**
 * Represents a table entry for a definition in SPL.
 *
 * There are three kinds of entries, that can be stored in a table.
 *
 * 1. Type Entries, that represent a type-definition and store a type
 *
 * 2. Variable Entries, that represent variable and parameter-definitions.
 ...
 *
 * 3. Procedure Entries, that represent a procedure-definition.
...
/**
 * Creates a new Type Entry representing a type-definition in SPL.
 * @param type The "meaning" of the type-definition.
 * It is determined by the type of the type expression on the right hand side of the definition.
 * @return A pointer to a newly created Entry in memory.
 */
Entry *newTypeEntry(Type *type);

/**
 * Creates a new Variable Entry representing a variable-definition in SPL.
 * @param type The semantic type of the variable. It is determined through the type of the definition.
...
/**
 * Creates a new Procedure Entry representing a procedure-definition in SPL.
